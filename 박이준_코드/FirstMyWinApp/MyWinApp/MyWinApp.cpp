...
namespace
{
	...
	LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		case WM_LBUTTONDOWN:
		{
			...
			int index = simplegeo::g_GeoShapeManager.FindShapeIndex(x, y);
			if (index != -1)
			{
				simplegeo::g_GeoShapeManager.RemoveShape(index);
			}
			else {
		...
		case WM_RBUTTONDOWN: // 우측 더블 클릭
		{
			std::cout << "WM_RBUTTONDBLCLK" << std::endl;
			// 마우스 오른쪽 버튼 클릭
			// 좌표를 가져온다.
			int x = LOWORD(lparam); // 위치 가져오기
			int y = HIWORD(lparam);
			int index = simplegeo::g_GeoShapeManager.FindShapeIndex(x, y);
			if (index != -1)
			{
				...
				simplegeo::g_GeoShapeManager.AddRectangle(x - 10, y - 10, x + 10, y + 10, RGB(255, 0, 0));
			}
...
{
	// GUI 창 생성
	HWND hWnd = CreateMyWindow(L"NzWindowClass", L"칠전칠기", 1280, 720);
