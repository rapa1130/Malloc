...
TCHAR* g_WindowName = _T("Mia::SW Renderer Tutorial T00 : Line 05");
...
void DrawFace()
{
	...
	// CW
	if (g_Vtx[0].x < g_Vtx[1].x)
	{
		// 직선 기울기 계산 (Y축 방향으로 진행되기 때문에 x/y)
		float sPa = (float)(g_Vtx[2].x - g_Vtx[0].x) / (g_Vtx[2].y - g_Vtx[0].y);
		float ePa = (float)(g_Vtx[1].x - g_Vtx[0].x)/ (g_Vtx[1].y - g_Vtx[0].y);
		// 점 위치 저장
		LONG pY = g_Vtx[0].y;
		LONG sP = g_Vtx[0].x;
		LONG eP = g_Vtx[0].x;

		// g_Vtx[1].y까지 기울기만큼 x값 가중하며 진행
		for (LONG h = 0; h < g_Vtx[1].y - g_Vtx[0].y; h++)
		{
			//SP 계산하기
			sP = g_Vtx[0].x + round(sPa*h);

			//EP 계산하기
			eP = g_Vtx[0].x + round(ePa *h);

			DrawHorizonLine(sP, eP, pY);
			pY++;
		}

		// g_Vtx[1].y부터 g_Vtx[2].y까지 기울기만큼 x값 가중하며 진행
		// eP쪽이 꺾이며 기울기 달라짐
		ePa = (float)(g_Vtx[2].x - g_Vtx[1].x) / (g_Vtx[2].y - g_Vtx[1].y);
		LONG middleSP = sP;
		for (LONG h = 0; h < g_Vtx[2].y - g_Vtx[1].y; h++)
		{
			//SP 계산하기
			sP = middleSP + round(sPa * h);

			//EP 계산하기
			eP = g_Vtx[1].x + round(ePa * h);
	...
	if (g_Vtx[0].x >= g_Vtx[1].x)
	{
		float sPa = (float)(g_Vtx[1].x - g_Vtx[0].x) / (g_Vtx[1].y - g_Vtx[0].y);
		float ePa = (float)(g_Vtx[2].x - g_Vtx[0].x) / (g_Vtx[2].y - g_Vtx[0].y);
		LONG pY = g_Vtx[0].y;
		LONG sP = g_Vtx[0].x;
		LONG eP = g_Vtx[0].x;
		...
		sPa = (float)(g_Vtx[2].x - g_Vtx[1].x) / (g_Vtx[2].y - g_Vtx[1].y);
		LONG middleEP = eP;
		for (LONG h = 0; h < g_Vtx[2].y - g_Vtx[1].y; h++)
		{
			//SP 계산하기
			sP = g_Vtx[1].x + round(sPa * h);

			//EP 계산하기
			eP = middleEP + round(ePa * h);
...
void ShowInfo()
{
	...
	DrawText(x, y += 16, col3, _T("4. 삼각형 그리기[사용자 함수 버전]"));
	DrawText(x, y += 16, col2, _T("5. 삼각형 채우기[사용자 함수 버전]"));
